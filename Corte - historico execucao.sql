...
to_ddmmyyyy(date(C.fecha_corte)) as fecha_corte,
to_ddmmyyyy(date(C.data_solic_corte)) as data_solic_corte,
b.descripcion as motivo_corte,
a.descricao_tipo as descricao_tipo,
f.descripcion as retorno_corte, 
to_ddmmyyyy(date(c.fecha_reposicion)) as fecha_reposicion,
d.descripcion as motivo_relig,
e.descripcion as retorno_relig, 
COUNT(*), 
AVG(valor_divida)
FROM CORREP C, outer tabla b, outer tabla f, outer tabla d, outer tipo_corte a, outer tabla e
WHERE b.nomtabla = 'CORMOT' And b.codigo = c.motivo_corte AND b.sucursal = '0000'  And ((b.fecha_activacion >= b.fecha_desactivac And b.fecha_desactivac > Today)  Or b.fecha_desactivac is null) 
